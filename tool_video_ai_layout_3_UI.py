...
class Ui_Widget(object):
    def setupUi(self, Widget):
        ...
        self.leftPanel.setFixedWidth(_s(290, sc))
        ...
        lv.setContentsMargins(_s(10,sc), _s(12,sc), _s(10,sc), _s(14,sc))
        lv.setSpacing(_s(8,sc))
        ...
        lv.addWidget(self._lbl("* Dùng cho nội dung lịch sử, chân dung", "noteLabel"))
        ...
        lv.addWidget(self._lbl("Copy X% nội dung video gốc, phần còn lại sáng tạo từ cấu trúc và thông điệp gốc", "noteLabel"))

        self.sw_duration = self._toggle_row("Tùy chỉnh thời lượng", True)
        lv.addWidget(self.sw_duration)
        lv.addWidget(self._lbl("⚠ Có thể gây mất chính xác thời lượng gốc", "warnLabel"))

        self.sw_ref_img    = self._toggle_row("Ảnh tham chiếu", True)
        self.sw_voice_uni  = self._toggle_row("Đồng nhất giọng nhân vật", True)
        self.sw_bgm        = self._toggle_row("Sinh nhạc nền (BGM)", True)
        self.sw_nsfw       = self._toggle_row("Hạn chế vi phạm ảnh tham chiếu", True)
        self.sw_auto_rot   = self._toggle_row("Chế độ tự động xoay profile NST", False)
        for sw in [self.sw_ref_img, self.sw_voice_uni, self.sw_bgm, self.sw_nsfw, self.sw_auto_rot]:
            lv.addWidget(sw)

        lv.addWidget(self._hline())
        lv.addWidget(self._lbl("TÙY CHỌN VIDEO", "sectionTitle"))

        lv.addWidget(self._lbl("Chọn luồng chạy (NST)", "grpLabel"))
        self.cb_threads = self._combo(["1 luồng", "2 luồng", "4 luồng", "8 luồng", "16 luồng"])
        lv.addWidget(self.cb_threads)

        lv.addWidget(self._lbl("Giọng nhân vật đồng nhất", "grpLabel"))
        self.cb_voice = self._combo(["Achenar - Female, soft", "Male Voice - Strong", "Child Voice"])
        lv.addWidget(self.cb_voice)

        lv.addWidget(self._lbl("Mô hình về ảnh tham chiếu", "grpLabel"))
        self.cb_img_model = self._combo(["🍌 Nano Banana Pro", "Gemini Flash", "DALL-E 3"])
        lv.addWidget(self.cb_img_model)
    ...
    def _combo(self, items):
        ...
        cb.addItems(items)
        cb.setFixedHeight(_s(34, self._sc))
        return cb

    def _le(self, placeholder="", password=False):
        le = QtWidgets.QLineEdit()
        le.setPlaceholderText(placeholder)
        le.setFixedHeight(_s(34, self._sc))
        if password:
